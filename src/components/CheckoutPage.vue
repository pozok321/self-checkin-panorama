    ...
    <div class="row">
      <div class="col-12 col-md-8 p-3 p-md-5">
        ...
        <div class="row">
          <div class="col-12">
            ...
            <span class="mb-3 font-eventbrief-sub d-block">{{
              event_detail.event_title
            }}</span>
            <div class="term-condition" v-html="event_detail.event_brief" style="color: #707070; font-size: 0.8rem">
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-12 col-md-4 p-3 p-md-5">
              ...
              <div class="form-group">
                <div class="input-group mb-3">
                  <Field type="text" class="form-control" name="promo_code" v-model="form_promoApply.coupon"
                    placeholder="Type Promo Code" :class="{ 'is-invalid': errors.promo_code }" />
        ...
        <div class="bg-grey checkout">
          ...
          <span class="span-checkout d-block mb-3">{{
            event_detail.event_title
          }}</span>
  ...
  export default {
    ...
    methods: {
      ...
      kFormatter(num, currency) {
        if (currency == "IDR") {
          return Math.abs(num) > 999 ? currency + " " + Math.sign(num) * (Math.abs(num) / 1000).toFixed(1) + "K" :
            currency + " " + Math.sign(num) * Math.abs(num);
        } else {
          return currency + value ? currency + " " + value.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,") : "";
        }
      },
